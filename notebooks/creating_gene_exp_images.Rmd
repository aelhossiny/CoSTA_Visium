...
```{r}
normal <- FindSpatiallyVariableFeatures(normal, assay = "SCT", features = VariableFeatures(normal),
    selection.method = "markvariogram")

top.features <- head(SpatiallyVariableFeatures(normal, selection.method = "markvariogram"), 6)
SpatialFeaturePlot(normal, features = top.features, ncol = 3, alpha = c(0.1, 1))
```
...
```{r}
fileConn<-file("/nfs/turbo/dcmb-class/bioinf593/sec001/hossiny/Bioinf593_project/results/SVG.txt")
writeLines(SpatiallyVariableFeatures(normal), fileConn)
close(fileConn)
```
...
for (i in 1:length(datasets_names)){
  ...
  dir.create(paste0("/nfs/turbo/dcmb-class/bioinf593/sec001/hossiny/Bioinf593_project/results/HVGeneExpImgs/",datasets_names[i]), showWarnings = FALSE)
  output_dir <- paste0("/nfs/turbo/dcmb-class/bioinf593/sec001/hossiny/Bioinf593_project/results/HVGeneExpImgs/",datasets_names[i])
